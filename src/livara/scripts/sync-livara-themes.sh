...
window, .background, .view, textview, entry, list, row,
...
headerbar, .titlebar, button, entry, list, row {
  background-color: $(json_color surface0);
}
/* Keep the popover/menu shell transparent; paint only its content nodes. */
popover, menu {
  background-color: transparent;
  border-color: $(json_color overlay0);
}
popover > contents, menu > menuitem {
  background-color: $(json_color surface0);
  color: $(json_color text);
}
popover > contents:hover, menu > menuitem:hover,
